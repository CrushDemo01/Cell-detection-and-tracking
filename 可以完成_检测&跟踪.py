...
from 多目标跟踪_re import mainTrack, NewTrack, MyTrack
...
def mainNMS(bounding_boxes, confidence_score):
    ...
    for (start_x, start_y, end_x, end_y), confidence in zip(bounding_boxes, confidence_score):
        ...
        # cv2.rectangle(org, (start_x, start_y - (2 * baseline + 5) + 10), (start_x + w, start_y+10), (0, 255, 255), -1)
        cv2.rectangle(org, (start_x, start_y), (start_x + w, start_y + 15), (0, 255, 255), -1)
        cv2.rectangle(org, (start_x, start_y), (end_x, end_y), (0, 255, 255), 1)
        cv2.putText(org, str(confidence), (start_x, start_y + 12), font, font_scale, (0, 0, 0), thickness)
    ...
    for (start_x, start_y, end_x, end_y), confidence in zip(picked_boxes, picked_score):
        (w, h), baseline = cv2.getTextSize(str(confidence), font, font_scale, thickness)
        # cv2.rectangle(image, (start_x, start_y - (2 * baseline + 5) + 10), (start_x + w, start_y+10), (0, 255, 255), -1)
...
def similarTargetDetection(frame_path):
    ...
    MyTrack(frame_path, boxes)
    # NewTrack(frame_path, boxes)
